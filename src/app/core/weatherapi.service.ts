...
export class WeatherapiService {
  ...
  isFavourityCity(data: WeatherClass) {
    ...
    //   }
    this.data1 = this.data1.map((currentValue: any) => {
      if (currentValue.id == data.id && currentValue.name == data.name) {
        fav = true;
        currentValue.isLiked = true;
      }
      return currentValue;
    });
    localStorage.setItem('favourite', JSON.stringify(this.data1));
